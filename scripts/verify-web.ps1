...
try {
    ...
    Assert-Condition ($appJs.Contains('Dependency lines:', [StringComparison]::Ordinal) -and $appJs.Contains('predecessor → successor', [StringComparison]::Ordinal)) 'Gantt must explain dependency connector direction in the visible controls.'
    Assert-Condition ($appJs.Contains('if (!state.gantt.showDependencies) return svg;', [StringComparison]::Ordinal) -and -not $appJs.Contains('if (!state.gantt.showDependencies && !isRelated) return;', [StringComparison]::Ordinal)) 'Gantt dependency connectors must be explicitly enabled rather than appearing only because a row is selected.'
    Assert-Condition ($appJs.Contains('path.appendChild(title)', [StringComparison]::Ordinal) -and $appJs.Contains('DEPENDENCY IMPACT', [StringComparison]::Ordinal) -and $appJs.Contains('Affects', [StringComparison]::Ordinal)) 'Gantt dependency impact must expose readable endpoint and downstream labels.'
    ...
    Assert-Condition ($stylesCss.Contains('stroke-linejoin: round', [StringComparison]::Ordinal) -and $stylesCss.Contains('.gantt-dependency-hint', [StringComparison]::Ordinal)) 'Gantt dependency connectors must use a readable stepped-line treatment and visible explanation.'
